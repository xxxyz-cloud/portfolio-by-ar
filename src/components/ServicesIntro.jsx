...
const ServicesIntro = () => {
  ...
  useGSAP(() => {
    const ctx = gsap.context(() => {
      ...
      /* ── Entrance timeline (auto-plays on load) ────── */
      ...
      /* ── Scroll exit (matchMedia) ────────────────────────────────
       *
       *  DESKTOP  — original pinned close (pin:true, +=180%)
       *  MOBILE   — NO pin. Circle closes while the section scrolls
       *             off the top naturally (over ~60 vh of scroll).
       *             On leave the video box is also hidden from the
       *             pointer/touch hit-test layer.
       * ─────────────────────────────────────────────────────────── */
      ...
      mm.add("(min-width: 768px)", () => {
        ...
        dtl.to(videoBoxRef.current, {
          clipPath: "circle(0% at 50% 50%)",
          duration: 1,
          ease:     "power2.inOut",
        });
        return () => dtl.scrollTrigger?.kill();
      });
      ...
      mm.add("(max-width: 767px)", () => {
        const mtl = gsap.timeline({
          scrollTrigger: {
            trigger: sectionRef.current,
            start:   "top top",
            end:     "+=60%",       // close circle over 60 vh of scroll
            scrub:   0.4,
            // ← deliberately no pin:true
            onLeave: () => {
              // Fully hide + remove from pointer/touch layer
        ...
        mtl.to(videoBoxRef.current, {
          clipPath: "circle(0% at 50% 50%)",
          ease:     "none",
        });
        return () => mtl.scrollTrigger?.kill();
      });
  ...
  return (
    ...
    >
      ...
      {/* Loading text — spreads apart as video appears */}
      <div
        ref={loadingRef}
      ...
      {/* Video box — starts tiny+rotated, expands full screen */}
      <div
        ref={videoBoxRef}
      ...
      >
        ...
        >
          ...
          <div ref={tagRowRef}
            className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 max-w-lg">
